...
#include "ActionTestPicture.h"
#include "Engine/World.h"
#include "Engine/Canvas.h"

FActionTestPicture::FActionTestPicture(UWorld* World)
{
	OwnerWorld = World;
...
void FActionTestPicture::Hide(float FadeOutTime)
{
...
bool FActionTestPicture::IsVisible() const
{
	return bIsVisible;
}

void FActionTestPicture::Tick(UCanvas * Canvas)
{
	if (bIsVisible)
	{
		...
		FCanvasTileItem TitleItem(FVector2D((Canvas->ClipX - width) / 2.0f, (Canvas->ClipY - height) / 2.0f), Image->Resource, FVector2D(width, height), TintColor);
		Canvas->DrawItem(TitleItem);
	}
}

void FActionTestPicture::Show(UTexture2D * InTexture, float InFadeInTime, float InScreenCoverage, bool bInKeepAspectRatio)
{
	if (InTexture != NULL)
	{
		Image = InTexture;
